...
public class YellowAI : TankAI
{
    ...
    void Start()
    {
        ...
        time_since_mine_lay = Random.Range(0.5f, 7f);
        live_bullets = 0;
    }
    ...
    void Update()
    {
        time_since_mine_lay -= Time.deltaTime;
        ...
        if (live_mines < max_mines && time_since_mine_lay < 0)
        {
            time_since_mine_lay = Random.Range(2f, 7f);
            mine.GetComponent<MineSpawner>().CreateMine(gameObject, transform.position);
        }

        if (time_since_direction_change < 0)
        {
            time_since_direction_change = Random.Range(0.5f, 3f);
